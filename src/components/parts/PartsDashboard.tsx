...
export function PartsDashboard({ session, onOpenSettings, onLogout }: PartsDashboardProps) {
  ...
  const refreshList = useCallback(async () => {
    ...
    try {
      const data = await api.listDepartmentRequests({ department: 'parts' });
      setRequests(data.requests);
    } catch (e: unknown) {
      const msg = e instanceof Error ? e.message : 'Failed to load parts inbox';
  ...
  return (
    <div className="benz-page">
      <div className="flex items-center justify-between gap-2 mb-4">
        <div className="flex items-center gap-2 text-sm text-benz-secondary">
          <Package size={18} className="text-benz-blue" />
          <span className="font-semibold text-benz-primary">Parts</span>
          <span>· {session.dealershipName}</span>
      ...
      {moduleDisabled ? (
        <div className="benz-card p-6 text-sm text-benz-secondary">
          <p className="font-semibold text-benz-primary mb-2">Parts module is disabled</p>
          <p>
            Ask a manager to enable the <strong>Parts Department</strong> module for this rooftop
            (Modules section), or set <code className="text-xs">MODULES_FORCE_ENABLE=parts</code>{' '}
        ...
        <DepartmentInbox
          ...
          onCreate={openCreate}
          emptyLabel="No parts requests yet — create one from a phone call or walk-in."
        />
      ) : mode === 'create' ? (
        <div>
          <button
          ...
          <h2 className="benz-page-title mb-4">New parts request</h2>
          {formFields}
